plot_pca <- function(filepath, nontarget = NULL, output, sgRNAcut = 30){
  if (is.null(output)){
    output = paste(getwd(), "/output/plot_pca", sep = "")
    dir.create(output)
  } else {
    output = paste(output, "/output/plot_pca", sep = "")
    dir.create(output)
  }
  ...
  pca <- stats::prcomp(t(sg.mt.nodup), scale. = TRUE)
  pca.x <- as.data.frame(pca$x[,1:2])
  pca.x$sgRNA <- rownames(pca.x)
  p1 <- ggplot2::ggplot(pca.x, aes(x=PC1,y=PC2,colour=barcode)) + ggplot2::geom_text(aes(x=PC1,y=PC2,label=barcode)) + ggplot2::geom_point() + ggplot2::theme_bw()
  pdf(paste(output,"/sgRNA_pca.pdf"), width = 20, height = 20)
  p1
  dev.off()
}
